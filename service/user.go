...
func checkUser(username, password string) bool {
	...
	// Search with a term query, select * from user where username = "" and password = ""
	...
	//create a query to check
	var tyu User
	for _, item := range queryResult.Each(reflect.TypeOf(tyu)) { //instanceOf
		u := item.(User)
...
func addUser(username, password string) bool {
	...
	// Search with a term query, check whether there exists the same user name;
	termQuery := elastic.NewTermQuery("username", username)
	queryResult, err := es_client.Search().
		Index(INDEX).
		Query(termQuery).
	...
	//Insert username and password into ES
	u := &User{
		Username:username,
		Password:password,
	}
...
func loginHandler(w http.ResponseWriter, r *http.Request) {
	...
	if checkUser(u.Username, u.Password) {
		...
		//convert it as map form
		claims := token.Claims.(jwt.MapClaims)
		/* Set token claims */
		claims["username"] = u.Username
		//Unitx() convert it as a long time format
